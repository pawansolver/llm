...
### Voice defaults and browser requirements

Voice features work without a paid provider: the default speech-to-text engine is local
faster-whisper using the `base` model with `int8` compute, and the default text-to-speech
engine is the browser Web Speech API. Administrators can change these persistent defaults
under **Admin Settings → Audio**, while each user can choose dictation or conversation mode,
language/automatic detection, silence handling, auto-send, auto-play, voice, playback speed,
and call interruption under **Settings → Audio**.

Microphone capture and browser speech APIs require a [secure context](https://developer.mozilla.org/docs/Web/Security/Secure_Contexts).
Use HTTPS in production. Plain HTTP is supported only on `localhost` (and browser-recognized
loopback origins); microphone access generally will not work from an HTTP LAN hostname or IP.
