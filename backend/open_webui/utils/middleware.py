...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # Built-in tool schemas can add thousands of input tokens to every request.
    # Allow cost-sensitive OpenAI-compatible connections (for example Groq's
    # free tier) to opt out without disabling tools for other providers.
    connection_builtin_tools = True
    url_idx = model.get('urlIdx')
    if url_idx is not None:
        openai_api_configs = await Config.get('openai.api_configs', {})
        connection_config = (openai_api_configs or {}).get(str(url_idx), {})
        connection_builtin_tools = connection_config.get('builtin_tools', True) is not False

    use_builtin_tools = (
        chat and (chat.meta or {}).get('internal') is True and (chat.meta or {}).get('type') == 'note'
    ) or (
        bool(metadata.get('session_id'))
        and connection_builtin_tools
