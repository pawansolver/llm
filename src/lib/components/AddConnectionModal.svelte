<script lang="ts">
	...
	const init = () => {
		if (connection) {
			...
			builtinTools = connection.config?.builtin_tools ?? true;
							...
							{#if !ollama && !direct}
								<div class="flex flex-row justify-between items-center w-full mt-3">
									<div class="flex flex-col">
										<label for="toggle-builtin-tools" class="text-xs text-gray-500">
											{$i18n.t('Built-in tools')}
										</label>
										<span class="text-[0.7rem] text-gray-400">
											{$i18n.t('Disable to reduce input-token usage for cost-sensitive APIs')}
										</span>
									</div>
									<Switch id="toggle-builtin-tools" bind:state={builtinTools} />
								</div>
							{/if}
