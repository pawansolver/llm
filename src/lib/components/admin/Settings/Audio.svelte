<script lang="ts">
	...
	let TTS_CACHE_TTL_SECONDS = 86400;
	let TTS_CACHE_MAX_ENTRIES = 1000;
	...
	let STT_ALLOWED_EXTENSIONS = '';
	let STT_MAX_UPLOAD_SIZE_MB = 20;
	let STT_MAX_CONCURRENT_REQUESTS = 2;
	let STT_TIMEOUT_SECONDS = 300;
	let STT_RATE_LIMIT = 20;
	...
	let audioHealth: any = null;
	let audioHealthLoading = false;
	...
	const updateConfigHandler = async () => {
		...
		const res = await updateAudioConfig(localStorage.token, {
			tts: {
				...
				CACHE_TTL_SECONDS: TTS_CACHE_TTL_SECONDS,
				CACHE_MAX_ENTRIES: TTS_CACHE_MAX_ENTRIES
			},
			stt: {
				...
				SUPPORTED_CONTENT_TYPES: STT_SUPPORTED_CONTENT_TYPES.split(',')
					...
					.filter(Boolean),
				ALLOWED_EXTENSIONS: STT_ALLOWED_EXTENSIONS.split(',')
					.map((value) => value.trim())
					.filter(Boolean),
				MAX_UPLOAD_SIZE_MB: STT_MAX_UPLOAD_SIZE_MB,
				MAX_CONCURRENT_REQUESTS: STT_MAX_CONCURRENT_REQUESTS,
				TIMEOUT_SECONDS: STT_TIMEOUT_SECONDS,
				RATE_LIMIT: STT_RATE_LIMIT,
	...
	const refreshHealth = async () => {
		audioHealthLoading = true;
		audioHealth = await getAudioHealth(localStorage.token).catch(() => null);
		audioHealthLoading = false;
	};
	...
	onMount(async () => {
		...
		if (res) {
			...
			TTS_CACHE_TTL_SECONDS = res.tts.CACHE_TTL_SECONDS ?? 86400;
			TTS_CACHE_MAX_ENTRIES = res.tts.CACHE_MAX_ENTRIES ?? 1000;
			...
			STT_ALLOWED_EXTENSIONS = (res?.stt?.ALLOWED_EXTENSIONS ?? []).join(',');
			STT_MAX_UPLOAD_SIZE_MB = res.stt.MAX_UPLOAD_SIZE_MB ?? 20;
			STT_MAX_CONCURRENT_REQUESTS = res.stt.MAX_CONCURRENT_REQUESTS ?? 2;
			STT_TIMEOUT_SECONDS = res.stt.TIMEOUT_SECONDS ?? 300;
			STT_RATE_LIMIT = res.stt.RATE_LIMIT ?? 20;
...
>
	...
	<div class="flex-1 min-h-0 overflow-y-auto scrollbar-hover pr-1.5">
		<AdminSettingSection title={$i18n.t('Health')} first>
			<AdminSettingRow
				label={$i18n.t('Audio Services')}
				description={$i18n.t('Readiness is checked without loading the local Whisper model.')}
			>
				<div class="flex items-center gap-2 text-xs">
					<span
						class={audioHealth?.status === 'ready'
							? 'text-green-600 dark:text-green-400'
							: 'text-amber-600 dark:text-amber-400'}
					>
						{audioHealthLoading
							? $i18n.t('Checking…')
							: $i18n.t(audioHealth?.status === 'ready' ? 'Ready' : 'Degraded')}
					</span>
					<button type="button" class="underline" on:click={refreshHealth}>
						{$i18n.t('Refresh')}
					</button>
				</div>
			</AdminSettingRow>
			{#if audioHealth}
				<div class="text-[0.6875rem] text-gray-500 dark:text-gray-400">
					{$i18n.t('STT')}: {audioHealth.stt_engine} · {$i18n.t('TTS')}:
					{audioHealth.tts_engine} · ffmpeg: {audioHealth.tools?.ffmpeg
						? $i18n.t('available')
						: $i18n.t('missing')} · {$i18n.t('Local model')}:
					{audioHealth.whisper?.loaded ? $i18n.t('loaded') : $i18n.t('lazy')}
				</div>
			{/if}
		</AdminSettingSection>

		<AdminSettingSection title={$i18n.t('Speech-to-Text')}>
			<AdminSettingRow
			...
			{#if STT_ENGINE !== 'web'}
				...
				<AdminSettingField
					...
					description={$i18n.t('Comma-separated extensions accepted for transcription uploads.')}
				>
					<input
						class={inputClass}
						bind:value={STT_ALLOWED_EXTENSIONS}
						placeholder="mp3,wav,m4a,webm,ogg,flac,mp4,mpga,mpeg"
					/>
				</AdminSettingField>
			{/if}
			...
			{:else if STT_ENGINE === ''}
				<AdminSettingField
					label={$i18n.t('STT Model')}
					description={$i18n.t(
						'Open WebUI uses faster-whisper locally; the default model is base with int8 compute.'
					)}
				>
		...
		<AdminSettingSection title={$i18n.t('Speech-to-Text Limits')}>
			<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
				<AdminSettingField label={$i18n.t('Maximum Upload Size (MB)')}>
					<input class={inputClass} type="number" min="1" bind:value={STT_MAX_UPLOAD_SIZE_MB} />
				</AdminSettingField>
				<AdminSettingField label={$i18n.t('Maximum Concurrent Requests')}>
					<input
						class={inputClass}
						type="number"
						min="1"
						bind:value={STT_MAX_CONCURRENT_REQUESTS}
					/>
				</AdminSettingField>
				<AdminSettingField label={$i18n.t('Transcription Timeout (seconds)')}>
					<input class={inputClass} type="number" min="1" bind:value={STT_TIMEOUT_SECONDS} />
				</AdminSettingField>
				<AdminSettingField label={$i18n.t('Requests Per Window')}>
					<input class={inputClass} type="number" min="0" bind:value={STT_RATE_LIMIT} />
				</AdminSettingField>
				<AdminSettingField label={$i18n.t('Rate Limit Window (seconds)')}>
					<input
						class={inputClass}
						type="number"
						min="1"
						bind:value={STT_RATE_LIMIT_WINDOW_SECONDS}
					/>
				</AdminSettingField>
				<AdminSettingField
					label={$i18n.t('Transcription Retention (seconds)')}
					description={$i18n.t('Use 0 to delete uploaded audio immediately after processing.')}
				>
					<input class={inputClass} type="number" min="0" bind:value={STT_RETENTION_SECONDS} />
				</AdminSettingField>
			</div>
		</AdminSettingSection>
		...
		<AdminSettingSection title={$i18n.t('Text-to-Speech')}>
			...
			>
				...
				>
					<option value="">{$i18n.t('Browser Web Speech (Free)')}</option>
			...
			<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
				<AdminSettingField label={$i18n.t('Speech Cache TTL (seconds)')}>
					<input class={inputClass} type="number" min="0" bind:value={TTS_CACHE_TTL_SECONDS} />
				</AdminSettingField>
				<AdminSettingField label={$i18n.t('Maximum Speech Cache Entries')}>
					<input class={inputClass} type="number" min="0" bind:value={TTS_CACHE_MAX_ENTRIES} />
				</AdminSettingField>
			</div>
		</AdminSettingSection>
	</div>
