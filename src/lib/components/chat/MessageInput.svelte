<script lang="ts">
	...
	import { completeDictation, joinDictationText } from '$lib/utils/dictation';
				...
				<div class="">
					...
					<div class={recording ? '' : 'hidden'}>
						<VoiceRecording
							...
							onCancel={async () => {
								recording = false;
								prompt = dictationDraft;
								chatInputElement?.setText(dictationDraft);

								await tick();
								document.getElementById('chat-input')?.focus();
							}}
							onInterim={({ text }) => {
								const dictatedPrompt = joinDictationText(dictationDraft, text);
								prompt = dictatedPrompt;
								chatInputElement?.setText(dictatedPrompt);
							}}
							onConfirm={async (data) => {
								const { text = '' } = data;
								const completed = completeDictation(
									dictationDraft,
									text,
								...
								chatInputElement?.setText(completed.text);
								await tick();
								document.getElementById('chat-input')?.focus();

								if (completed.shouldSubmit) {
									dispatch('submit', completed.text);
								}
							}}
						/>
					</div>
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									...
									{:else}
											...
											{#if $_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)}
												...
												<Tooltip content={$i18n.t('Dictate')}>
													<button
														...
														type="button"
														on:click={async () => {
															dictationDraft = prompt;
															recording = true;
														}}
														aria-label={$i18n.t('Start voice dictation')}
													>
										...
										{#if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												<!-- {$i18n.t('Call')} -->
												<Tooltip content={$i18n.t('Voice mode')} touch={false}>
													<button
