<script lang="ts">
	...
	let STTEngine = '';
	let STTLanguage = '';
	let STTVadEnabled = false;
	let STTSilenceDurationMs = 2000;
	let STTVadThreshold = 0.02;
	...
	onMount(async () => {
		...
		STTVadEnabled = $settings?.audio?.stt?.vad?.enabled ?? false;
		STTSilenceDurationMs = $settings?.audio?.stt?.vad?.silenceDurationMs ?? 2000;
		STTVadThreshold = $settings?.audio?.stt?.vad?.threshold ?? 0.02;
		...
		saveSettings({
			audio: {
				stt: {
					...
					vad: {
						enabled: STTVadEnabled,
						silenceDurationMs: STTSilenceDurationMs,
						threshold: STTVadThreshold
					}
				},
				tts: {
...
>
	...
	<div class="flex-1 min-h-0 overflow-y-auto scrollbar-hover pr-1.5">
		<div class="mb-3 text-[0.6875rem] text-gray-500 dark:text-gray-400">
			{$i18n.t(
				'Microphone features require HTTPS or localhost because browsers only expose them in a secure context.'
			)}
		</div>

		{#if $user?.role === 'admin' || ($user?.permissions?.chat?.call ?? true)}
			<UserSettingSection title={$i18n.t('Voice Interaction')} first>
				<UserSettingRow
					label={$i18n.t('Voice Input Mode')}
					description={$i18n.t(
						'Dictation fills the message box; conversation mode continues listening after spoken responses.'
					)}
				>
					<SettingsSelect
						bind:value={conversationMode}
						ariaLabel={$i18n.t('Voice Input Mode')}
						on:change={() => saveSettings({ conversationMode })}
					>
						<option value={false}>{$i18n.t('Dictation')}</option>
						<option value={true}>{$i18n.t('Conversation')}</option>
					</SettingsSelect>
				</UserSettingRow>
				<UserSettingRow
					label={$i18n.t('Allow Voice Interruption in Call')}
					description={$i18n.t('Let speech interrupt the assistant during a voice call.')}
				>
					<Switch
						bind:state={voiceInterruption}
						ariaLabel={$i18n.t('Allow Voice Interruption in Call')}
		...
		{#if $user?.role === 'admin' || ($user?.permissions?.chat?.stt ?? true)}
			<UserSettingSection
				title={$i18n.t('STT Settings')}
				first={!($user?.role === 'admin' || ($user?.permissions?.chat?.call ?? true))}
			>
				{#if $config.audio.stt.engine !== 'web'}
					...
					>
						...
						>
							<option value="">{$i18n.t('Server Default (Local Whisper)')}</option>
				...
				<UserSettingRow
					label={$i18n.t('Stop Dictation After Silence')}
					description={$i18n.t(
						'Automatically finish dictation after speech followed by the configured period of silence.'
					)}
				>
					<Switch bind:state={STTVadEnabled} ariaLabel={$i18n.t('Stop Dictation After Silence')} />
				</UserSettingRow>

				{#if STTVadEnabled}
					<UserSettingRow
						label={$i18n.t('Silence Duration')}
						description={$i18n.t('How long to wait after speech before finishing dictation.')}
					>
						<div class="flex items-center gap-1 text-xs text-gray-400">
							<input
								...
								max="30000"
								step="100"
								bind:value={STTSilenceDurationMs}
								aria-label={$i18n.t('Silence duration in milliseconds')}
								class="h-7 w-20 rounded-lg border border-gray-100/50 bg-gray-50/40 px-2 text-right text-xs text-gray-700 outline-hidden focus:border-blue-400 dark:border-white/[0.04] dark:bg-white/[0.03] dark:text-gray-300"
							/>
							<span>{$i18n.t('ms')}</span>
						</div>
					</UserSettingRow>
					...
					<UserSettingRow
						label={$i18n.t('Voice Activity Threshold')}
						description={$i18n.t(
							'Increase this value if background noise prevents dictation from stopping.'
						)}
					>
						<input
							type="number"
							...
							max="1"
							step="0.001"
							bind:value={STTVadThreshold}
							aria-label={$i18n.t('Voice activity threshold')}
							class="h-7 w-20 rounded-lg border border-gray-100/50 bg-gray-50/40 px-2 text-right text-xs text-gray-700 outline-hidden focus:border-blue-400 dark:border-white/[0.04] dark:bg-white/[0.03] dark:text-gray-300"
						/>
					</UserSettingRow>
				{/if}
