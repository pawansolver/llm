<script lang="ts">
	...
	const dispatchCallOverlayAudio = (message, final = false) => {
		...
		const dispatchedCount = message.callAudioPartCount ?? 0;
		messageContentParts.slice(dispatchedCount).forEach((content) => {
		...
		message.callAudioPartCount = messageContentParts.length;
	};
